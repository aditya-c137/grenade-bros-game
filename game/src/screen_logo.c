...
void DrawLogoScreen(void)
{
    ...
    else if (state == 3)    // Draw "raylib" text-write animation + "powered by"
    {
        ...
        DrawText(TextSubtext("boobas", 0, lettersCount), GetScreenWidth()/2 - 68, GetScreenHeight()/2 + 68, 50, Fade(BLACK, alpha));

        if (framesCounter > 20) DrawText("welcome to ;)", logoPositionX, logoPositionY - 27, 20, Fade(DARKGRAY, alpha));
    }
}
